...
function DocPage({ intl }: { intl: IntlShape }) {
  ...
  const imagePreviewData: PropsTableData[] = [
    ...
    },
    {
      name: "actions",
      description: formatMessage({
        defaultMessage: "额外操作，[ImagePreviewActionProps](#image-preview-action-props)。",
      }),
  ...
  const imagePreviewGroupData: PropsTableData[] = [
    ...
    },
    {
      name: "srcList",
      description: formatMessage({
        defaultMessage: "图片列表 （设置了本属性之后，将不再收集 Image 子组件的图片信息）。",
      }),
      type: "string[]",
    },
    {
  ...
  return (
    <Unstyled>
      ...
      <Paragraph>{formatMessage({ defaultMessage: "展示和预览图片。" })}</Paragraph>
      ...
      <Paragraph>{formatMessage({ defaultMessage: "当加载图片失败的时候显示的内容。" })}</Paragraph>
      ...
      <Paragraph>
        {formatMessage({
          defaultMessage: "用 Image.PreviewGroup 包裹 Image 组件即可进行多图预览。",
        })}
      </Paragraph>
      ...
      <Paragraph>
        {formatMessage({
          defaultMessage: "Image.Preview 可单独使用，需要配置 src，并控制 visible。",
        })}
      </Paragraph>
      ...
      <Source code={`import { Image, type ImageProps } from "growingio-design";`} language="tsx" />
