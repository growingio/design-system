...
function DocPage({ intl }: { intl: IntlShape }) {
  ...
  const data: PropsTableData[] = [
    ...
    },
    {
      name: "colon",
      description: formatMessage({
        defaultMessage: "是否显示标签后的一个冒号，优先级小于 `Form.Item` 中 `colon` 的优先级。",
      }),
  ...
  const formItemData = [
    ...
    },
    {
      name: "initialValue",
      description: formatMessage({
        defaultMessage: "设置控件初始值.(初始值,请不要使用受控组件的defaultValue了)",
      }),
      type: "FieldValue",
    },
    {
    ...
    },
    {
      name: "formatter",
      description: formatMessage({
        defaultMessage: "将Form内保存的当前控件对应的值进行一定的转换,再传递给控件。",
      }),
  ...
  return (
    <Unstyled>
      ...
      <Paragraph>
        {formatMessage({
          defaultMessage: "Form 支持三种排列方式：水平排列 (默认)、垂直排列、行内排列。",
        })}
      </Paragraph>
      ...
      <Source code={`import { Form, type FormProps } from "growingio-design";`} language="tsx" />
