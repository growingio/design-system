...
function DocPage({ intl }: { intl: IntlShape }) {
  ...
  const data: PropsTableData[] = [
    ...
    },
    {
      name: "error",
      description: formatMessage({
        defaultMessage: "是否是错误状态。(废弃，下个大版本移除，使用 status='error' 替代)",
      }),
      type: "boolean",
    },
    {
  ...
  return (
    <Unstyled>
      ...
      <Paragraph>{formatMessage({ defaultMessage: "通过设置 size 来指定大小。" })}</Paragraph>
      ...
      <Paragraph>
        {formatMessage({
          defaultMessage: "设置 saveOnBlur 在失焦时自动将正在输入的文本保存为标签。",
        })}
      </Paragraph>
      ...
      <Paragraph>
        {formatMessage({
          defaultMessage: "指定 dragToSort 属性以允许对已输入的值进行拖拽排序。",
        })}
      </Paragraph>
      ...
      <Source code={`export type ValueChangeReason = "add" | "remove" | "clear" | "sort";`} />
    </Unstyled>
  );
}
